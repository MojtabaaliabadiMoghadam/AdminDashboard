...
<!--  </Dialog>-->
  <Dialog v-model:visible="model" modal :showHeader="false" :dismissable-mask="true" pt:root:class="['max-w-7xl']">
    <div class=" sm:p-7 sm:pb-7">
      <div class="flex items-center ps-3 justify-between">
        <div class="flex items-center gap-x-1">
          <slot name="before_label"/>
          <h3  class="">{{ title }}</h3>
          <slot name="extra_label"/>
        </div>
        <div  @click="handleClose" class="h-12 w-12 flex justify-center rounded-full hover:bg-blue-100 transition-all duration-200 ease-in items-center">
    ...
    <form @submit.prevent="confirmAction">
      <div v-if="$slots.body" class="bg-white px-7 pb-7">
        <div  style="max-height: calc(100vh - 350px)">
          <slot name="body"></slot>
        </div>
      </div>
      <div class=" sm:flex w-full sm:flex-row-reverse pt-9 pb-7 sm:px-7">
